...
export function execFileWithShim(cmd: string, args: string[], options: SpawnSyncOptions = {}): string {
  const result = spawnSync(cmd, args, { ...options, stdio: ['ignore', 'pipe', 'pipe'] })
  if (result.error && (result.error as NodeJS.ErrnoException).code === 'ENOENT' && process.platform === 'win32') {
    ...
    const shellResult = spawnSync(shellCmd, [], { ...options, shell: true, stdio: ['ignore', 'pipe', 'pipe'] })
    if (shellResult.error) throw shellResult.error
    const code = shellResult.status ?? 1
    if (code !== 0) {
      const e = new Error(shellResult.stderr?.toString().trim() || `exit code ${code}`) as Error & { status: number; stderr: string; stdout: string }
      e.status = code
      e.stderr = shellResult.stderr?.toString() || ''
      e.stdout = shellResult.stdout?.toString() || ''
      throw e
    }
    return shellResult.stdout?.toString() ?? ''
  }
