...
import { delegate, delegateFile, delegateShell, delegateStdout } from '../src/execute/delegate.js'
...
import type { ExecuteResult, ExecutionPlan, HostAgentInfo } from '../src/execute/types.js'
...
describe('buildExecutionPlan', () => {
  it('keeps a lint/human-review gate empty (no vitest run fallback)', () => {
    const plan = buildExecutionPlan('add feature', 'feature', 1, {
      ...
      tier: 'any',
      trust: 'human',
      steps: ['ask', 'research', 'present-options'],
      checks: ['human-review'],
    })
