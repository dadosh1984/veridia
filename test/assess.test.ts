...
describe('probeOracles', () => {
  ...
  it('detects a test runner from a package.json test script', () => {
    const target = makeTmpDir()
    const found = probeOracles(target, fakeFs(target, { 'package.json': '{"scripts":{"test":"vitest run"}}' }))
    expect(found).toContainEqual<Oracle>({ kind: 'test-runner' })
  })
  ...
  it('detects a type-check from a BOM-prefixed package.json typecheck script', () => {
    const target = makeTmpDir()
    const found = probeOracles(target, fakeFs(target, { 'package.json': '\uFEFF{"scripts":{"typecheck":"tsc"}}' }))
  ...
  it('detects a ci oracle from a github workflow yaml', () => {
    const target = makeTmpDir()
    const found = probeOracles(target, fakeFs(target, { '.github/workflows/ci.yml': 'name: ci' }))
  ...
  it('never probes into node_modules', () => {
    const target = makeTmpDir()
    const found = probeOracles(target, fakeFs(target, { 'node_modules/vitest.config.ts': '', 'node_modules/tsconfig.json': '' }))
    expect(found).toEqual<Oracle[]>([])
  })
